...
// Query untuk mengambil rakernas
$sql = "SELECT * FROM rakernas";
...
<head>
  ...
  <title>Rakernas | Himapolindo</title>
...
<body>
  <div class="wrapper">
        ...
        <div class="row">
          <div class="col-12">
            <div class="col-sm-12 col-md-12 col-lg-6 offset-lg-3">
              <h1 class="pagetitle__heading mb-0" style="color: white;">Rakernas</h1>
            </div>
          </div><!-- /.col-xl-6 -->
        </div><!-- /.row -->
      </div><!-- /.container -->
    ...
    // Query untuk mengambil username dari tabel login berdasarkan id_login dari tabel rakernas
...
WHERE r.id_rakernas = ?";
    ...
    <div class="container mt-60 mb-50">
      ...
      <div class="row">
        ...
        $order_by = '';
        if ($sort == 'asc') {
          $order_by = 'ORDER BY tanggal_rakernas ASC';
        } elseif ($sort == 'desc') {
          $order_by = 'ORDER BY tanggal_rakernas DESC';
        ...
        $query = "SELECT r.*, l.username
              FROM rakernas r
              LEFT JOIN login l ON r.id_login = l.id_login
              WHERE LOWER(judul_rakernas) LIKE LOWER('%$search%') $order_by
              LIMIT $items_per_page OFFSET $offset";
        $result = $conn->query($query);

        // Query untuk mendapatkan jumlah total data (untuk pagination)
        $queryTotal = "SELECT COUNT(*) AS total FROM rakernas WHERE LOWER(judul_rakernas) LIKE LOWER('%$search%')";
        $resultTotal = $conn->query($queryTotal);
        $total = $resultTotal->fetch_assoc()['total'];

        // Hitung total jumlah data
        $total_items_query = "SELECT COUNT(*) as count FROM rakernas WHERE LOWER(judul_rakernas) LIKE LOWER('%$search%')";
        ...
        if ($result->num_rows > 0) {
          ...
          // Jika tidak ada data
          echo '<div class="col-12"><p class="text-center">Tidak ada data rakernas.</p></div>';
        }
        ?>
      </div>
    </div>
  ...
  <script>
    function increaseViews(id) {
      $.ajax({
        url: "increase_views_rakernas.php",
        type: "POST",
        data: {
          id_rakernas: id
        },
