...
// Query untuk mengambil kongres
$sql = "SELECT * FROM kongres";
...
<head>
  ...
  <title>Kongres | Himapolindo</title>
...
<body>
  <div class="wrapper">
        ...
        <div class="row">
          <div class="col-12">
            <div class="col-sm-12 col-md-12 col-lg-6 offset-lg-3">
              <h1 class="pagetitle__heading mb-0" style="color: white;">Kongres Himapolindo</h1>
            </div>
          </div><!-- /.col-xl-6 -->
        </div><!-- /.row -->
      </div><!-- /.container -->
    ...
    // Query untuk mengambil username dari tabel login berdasarkan id_login dari tabel kongres
...
WHERE r.id_kongres = ?";
    ...
    <div class="container mt-60 mb-50">
      ...
      <div class="row">
        ...
        $order_by = '';
        if ($sort == 'asc') {
          $order_by = 'ORDER BY tanggal_kongres ASC';
        } elseif ($sort == 'desc') {
          $order_by = 'ORDER BY tanggal_kongres DESC';
        ...
        $query = "SELECT r.*, l.username
              FROM kongres r
              LEFT JOIN login l ON r.id_login = l.id_login
              WHERE LOWER(judul_kongres) LIKE LOWER('%$search%') $order_by
              LIMIT $items_per_page OFFSET $offset";
        $result = $conn->query($query);

        // Query untuk mendapatkan jumlah total data (untuk pagination)
        $queryTotal = "SELECT COUNT(*) AS total FROM kongres WHERE LOWER(judul_kongres) LIKE LOWER('%$search%')";
        $resultTotal = $conn->query($queryTotal);
        $total = $resultTotal->fetch_assoc()['total'];

        // Hitung total jumlah data
        $total_items_query = "SELECT COUNT(*) as count FROM kongres WHERE LOWER(judul_kongres) LIKE LOWER('%$search%')";
        ...
        if ($result->num_rows > 0) {
          ...
          // Jika tidak ada data
          echo '<div class="col-12"><p class="text-center">Tidak ada data kongres.</p></div>';
        }
        ?>
      </div>
    </div>
  ...
  <script>
    function increaseViews(id) {
      $.ajax({
        url: "increase_views_kongres.php",
        type: "POST",
        data: {
          id_kongres: id
        },
