...
<head>
  ...
  <title>Artikel | Himapolindo</title>
...
<body>
  <div class="wrapper">
    ...
    // Query untuk mengambil username dari tabel login berdasarkan id_login dari tabel artikel
...
WHERE r.id_artikel = ?";
    ...
    <div class="container mt-60 mb-50">
      ...
      <div class="row">
        ...
        $order_by = '';
        if ($sort == 'asc') {
          $order_by = 'ORDER BY tanggal_artikel ASC';
        } elseif ($sort == 'desc') {
          $order_by = 'ORDER BY tanggal_artikel DESC';
        ...
        $query = "SELECT r.*, l.username
              FROM artikel r
              LEFT JOIN login l ON r.id_login = l.id_login
              WHERE LOWER(judul_artikel) LIKE LOWER('%$search%') $order_by
              LIMIT $items_per_page OFFSET $offset";
        $result = $conn->query($query);

        // Query untuk mendapatkan jumlah total data (untuk pagination)
        $queryTotal = "SELECT COUNT(*) AS total FROM artikel WHERE LOWER(judul_artikel) LIKE LOWER('%$search%')";
        $resultTotal = $conn->query($queryTotal);
        $total = $resultTotal->fetch_assoc()['total'];

        // Hitung total jumlah data
        $total_items_query = "SELECT COUNT(*) as count FROM artikel WHERE LOWER(judul_artikel) LIKE LOWER('%$search%')";
        ...
        if ($result->num_rows > 0) {
          ...
          // Jika tidak ada data
          echo '<div class="col-12"><p class="text-center">Tidak ada data artikel.</p></div>';
        }
        ?>
      </div>
    </div>
