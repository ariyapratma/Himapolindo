...
$id_diesnatalis = $_GET['id'];
$judul_diesnatalis = "";
$deskripsi = "";
$thumbnail_diesnatalis = "";
$update_success = false;

// Proses ambil data Dies Natalis Himapolindo dari database
$sql = "SELECT * FROM diesnatalis WHERE id_diesnatalis = ?";
...
if ($result->num_rows > 0) {
    ...
    $judul_diesnatalis = $row['judul_diesnatalis'];
    $deskripsi = strip_tags($row['konten_diesnatalis']);
    $thumbnail_diesnatalis = $row['thumbnail_diesnatalis'];
} else {
    $_SESSION['flash_message'] = "Data tidak ditemukan.";
    $_SESSION['flash_type'] = "error";
    header("Location: lihat_dies_natalis.php");
    exit();
}

// Proses update data Dies Natalis Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $judul_diesnatalis = $_POST['judul_diesnatalis'];
    $deskripsi = $_POST['deskripsi'];

    // Proses file upload thumbnail jika ada perubahan
    if ($_FILES["userfile"]["size"][0] > 0) {
        $target_dir = "../../assets_dashboard/images/uploads/thumbnail_dies_natalis/";
        ...
        if ($uploadOk == 0) {
            header("Location: edit_dies_natalis.php?id=$id_diesnatalis");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE diesnatalis SET judul_diesnatalis = ?, thumbnail_diesnatalis = ?, konten_diesnatalis = ? WHERE id_diesnatalis = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_diesnatalis, $target_file, $deskripsi, $id_diesnatalis);
            } else {
        ...
        $sql_update = "UPDATE diesnatalis SET judul_diesnatalis = ?, konten_diesnatalis = ? WHERE id_diesnatalis = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_diesnatalis, $deskripsi, $id_diesnatalis);
    }
...
<head>
    <meta charset="utf-8" />
    <title>Edit Dies Natalis | Himapolindo</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Dies Natalis</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="lihat_dies_natalis.php">Lihat Dies Natalis</a></li>
                                        <li class="breadcrumb-item active">Edit Dies Natalis</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="edit_dies_natalis.php?id=<?php echo $id_diesnatalis; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="judul_diesnatalis">Judul Dies Natalis</label>
                                            <input type="text" class="form-control" id="judul_diesnatalis" name="judul_diesnatalis" value="<?php echo htmlspecialchars($judul_diesnatalis); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="thumbnail_diesnatalis">Thumbnail</label>
                                            <input type="file" class="form-control" id="userfile" name="userfile[]">
                                            <?php if ($thumbnail_diesnatalis) : ?>
                                                <img src="<?php echo $thumbnail_diesnatalis; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="lihat_dies_natalis.php" class="btn btn-secondary">Kembali</a>
                                    </form>
                                </div>
                            </div>
                        </div>
