...
// Proses ambil data Artikel Himapolindo dari database
...
if ($result->num_rows > 0) {
    ...
    $deskripsi = strip_tags($row['konten_artikel']);
    $thumbnail_artikel = $row['thumbnail_artikel'];
} else {
...
// Proses update data Artikel Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($_FILES["userfile"]["size"][0] > 0) {
        $target_dir = "../../assets_dashboard/images/uploads/thumbnail_artikel/";
        ...
        if ($uploadOk == 0) {
            header("Location: edit_artikel.php?id=$id_artikel");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE artikel SET judul_artikel = ?, thumbnail_artikel = ?, konten_artikel = ? WHERE id_artikel = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_artikel, $target_file, $deskripsi, $id_artikel);
            } else {
        ...
        $sql_update = "UPDATE artikel SET judul_artikel = ?, konten_artikel = ? WHERE id_artikel = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_artikel, $deskripsi, $id_artikel);
    }
                    ...
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Artikel</h4>
                                ...
                                <div class="card-body">
                                    <form method="POST" action="edit_artikel.php?id=<?php echo $id_artikel; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="judul_artikel" name="judul_artikel" value="<?php echo htmlspecialchars($judul_artikel); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            ...
                                            <?php if ($thumbnail_artikel) : ?>
                                                <img src="<?php echo $thumbnail_artikel; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
