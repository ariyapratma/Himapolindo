...
// Proses ambil data Kongres Himapolindo dari database
$sql = "SELECT * FROM kongres WHERE id_kongres = ?";
...
if ($result->num_rows > 0) {
    ...
    $judul_kongres = $row['judul_kongres'];
    $deskripsi = strip_tags($row['konten_kongres']);
    $thumbnail_kongres = $row['thumbnail_kongres'];
} else {
    $_SESSION['flash_message'] = "Data tidak ditemukan.";
    $_SESSION['flash_type'] = "error";
    header("Location: lihat_kongres.php");
    exit();
}

// Proses update data Kongres Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $judul_kongres = $_POST['judul_kongres'];
    $deskripsi = $_POST['deskripsi'];

    // Proses file upload thumbnail jika ada perubahan
    if ($_FILES["userfile"]["size"][0] > 0) {
        $target_dir = "../../assets_dashboard/images/uploads/thumbnail_kongres/";
        ...
        if ($uploadOk == 0) {
            header("Location: edit_kongres.php?id=$id_kongres");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE kongres SET judul_kongres = ?, thumbnail_kongres = ?, konten_kongres = ? WHERE id_kongres = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_kongres, $target_file, $deskripsi, $id_kongres);
            } else {
        ...
        $sql_update = "UPDATE kongres SET judul_kongres = ?, konten_kongres = ? WHERE id_kongres = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_kongres, $deskripsi, $id_kongres);
    }
...
<head>
    <meta charset="utf-8" />
    <title>Edit Kongres | Himapolindo</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Kongres Himapolindo</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="lihat_kongres.php">Lihat Kongres Himapolindo</a></li>
                                        <li class="breadcrumb-item active">Edit Kongres Himapolindo</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="edit_kongres.php?id=<?php echo $id_kongres; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="judul_kongres">Judul Kongres Himapolindo</label>
                                            <input type="text" class="form-control" id="judul_kongres" name="judul_kongres" value="<?php echo htmlspecialchars($judul_kongres); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="thumbnail_kongres">Thumbnail</label>
                                            <input type="file" class="form-control" id="userfile" name="userfile[]">
                                            <?php if ($thumbnail_kongres) : ?>
                                                <img src="<?php echo $thumbnail_kongres; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="lihat_kongres.php" class="btn btn-secondary">Kembali</a>
                                    </form>
                                </div>
                            </div>
                        </div>
