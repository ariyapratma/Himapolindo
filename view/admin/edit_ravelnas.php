...
// Proses ambil data Ravelnas Himapolindo dari database
$sql = "SELECT * FROM ravelnas WHERE id_ravelnas = ?";
...
if ($result->num_rows > 0) {
    ...
    $judul_ravelnas = $row['judul_ravelnas'];
    $deskripsi = strip_tags($row['konten_ravelnas']);
    $thumbnail_ravelnas = $row['thumbnail_ravelnas'];
} else {
    $_SESSION['flash_message'] = "Data tidak ditemukan.";
    $_SESSION['flash_type'] = "error";
    header("Location: lihat_ravelnas.php");
    exit();
}

// Proses update data Ravelnas Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $judul_ravelnas = $_POST['judul_ravelnas'];
    $deskripsi = $_POST['deskripsi'];

    // Proses file upload thumbnail jika ada perubahan
    if ($_FILES["userfile"]["size"][0] > 0) {
        $target_dir = "../../assets_dashboard/images/uploads/thumbnail_ravelnas/";
        ...
        if ($uploadOk == 0) {
            header("Location: edit_ravelnas.php?id=$id_ravelnas");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE ravelnas SET judul_ravelnas = ?, thumbnail_ravelnas = ?, konten_ravelnas = ? WHERE id_ravelnas = ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_ravelnas, $target_file, $deskripsi, $id_ravelnas);
            } else {
        ...
        $sql_update = "UPDATE ravelnas SET judul_ravelnas = ?, konten_ravelnas = ? WHERE id_ravelnas = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_ravelnas, $deskripsi, $id_ravelnas);
    }
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Ravelnas</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="lihat_ravelnas.php">Lihat Ravelnas</a></li>
                                        <li class="breadcrumb-item active">Edit Ravelnas</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="edit_ravelnas.php?id=<?php echo $id_ravelnas; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="judul_ravelnas">Judul Ravelnas</label>
                                            <input type="text" class="form-control" id="judul_ravelnas" name="judul_ravelnas" value="<?php echo htmlspecialchars($judul_ravelnas); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="thumbnail_ravelnas">Thumbnail</label>
                                            <input type="file" class="form-control" id="userfile" name="userfile[]">
                                            <?php if ($thumbnail_ravelnas) : ?>
                                                <img src="<?php echo $thumbnail_ravelnas; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="lihat_ravelnas.php" class="btn btn-secondary">Kembali</a>
                                    </form>
                                </div>
                            </div>
                        </div>
