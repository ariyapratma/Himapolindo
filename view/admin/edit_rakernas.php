...
// Proses ambil data Rakernas Himapolindo dari database
$sql = "SELECT * FROM rakernas WHERE id_rakernas = ?";
...
if ($result->num_rows > 0) {
    ...
    $judul_rakernas = $row['judul_rakernas'];
    $deskripsi = strip_tags($row['konten_rakernas']);
    $thumbnail_rakernas = $row['thumbnail_rakernas'];
} else {
    $_SESSION['flash_message'] = "Data tidak ditemukan.";
    $_SESSION['flash_type'] = "error";
    header("Location: lihat_rakernas.php");
    exit();
}

// Proses update data Rakernas Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $judul_rakernas = $_POST['judul_rakernas'];
    $deskripsi = $_POST['deskripsi'];

    // Proses file upload thumbnail jika ada perubahan
    if ($_FILES["userfile"]["size"][0] > 0) {
        $target_dir = "../../assets_dashboard/images/uploads/thumbnail_rakernas";
        ...
        if ($uploadOk == 0) {
            header("Location: edit_rakernas.php?id=$id_rakernas");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE rakernas SET judul_rakernas = ?, thumbnail_rakernas = ?, konten_rakernas = ? WHERE id_rakernas= ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_rakernas, $target_file, $deskripsi, $id_rakernas);
            } else {
        ...
        $sql_update = "UPDATE rakernas SET judul_rakernas = ?, konten_rakernas = ? WHERE id_rakernas = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_rakernas, $deskripsi, $id_rakernas);
    }
...
<head>
    <meta charset="utf-8" />
    <title>Edit Rakernas | Himapolindo</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Rakernas</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="lihat_rakernas.php">Lihat Rakernas</a></li>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="edit_rakernas.php?id=<?php echo $id_rakernas; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="judul_rakernas">Judul Rakernas</label>
                                            <input type="text" class="form-control" id="judul_rakernas" name="judul_rakernas" value="<?php echo htmlspecialchars($judul_rakernas); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="thumbnail_rakernas">Thumbnail</label>
                                            <input type="file" class="form-control" id="userfile" name="userfile[]">
                                            <?php if ($thumbnail_rakernas) : ?>
                                                <img src="<?php echo $thumbnail_rakernas; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="lihat_rakernas.php" class="btn btn-secondary">Kembali</a>
                                    </form>
                                </div>
                            </div>
                        </div>
