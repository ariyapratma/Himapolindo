...
// Proses ambil data Calek Himapolindo dari database
$sql = "SELECT * FROM calek WHERE id_calek = ?";
...
if ($result->num_rows > 0) {
    ...
    $judul_calek = $row['judul_calek'];
    $deskripsi = strip_tags($row['konten_calek']);
    $thumbnail_calek = $row['thumbnail_calek'];
} else {
    $_SESSION['flash_message'] = "Data tidak ditemukan.";
    $_SESSION['flash_type'] = "error";
    header("Location: lihat_catatan.php");
    exit();
}

// Proses update data Clek Himapolindo
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    $judul_calek = $_POST['judul_calek'];
    ...
    if ($_FILES["userfile"]["size"][0] > 0) {
        ...
        if ($uploadOk == 0) {
            header("Location: edit_catatan.php?id=$id_calek");
            exit();
        } else {
            if (move_uploaded_file($_FILES["userfile"]["tmp_name"][0], $target_file)) {
                // Update data dengan thumbnail baru
                $sql_update = "UPDATE calek SET judul_calek = ?, thumbnail_calek = ?, konten_calek = ? WHERE id_calek= ?";
                $stmt_update = $conn->prepare($sql_update);
                $stmt_update->bind_param("sssi", $judul_calek, $target_file, $deskripsi, $id_calek);
            } else {
        ...
        $sql_update = "UPDATE calek SET judul_calek = ?, konten_calek = ? WHERE id_calek = ?";
        $stmt_update = $conn->prepare($sql_update);
        $stmt_update->bind_param("ssi", $judul_calek, $deskripsi, $id_calek);
    }
...
<head>
    <meta charset="utf-8" />
    <title>Edit Catatan Intelektual | Himapolindo</title>
                ...
                <div class="container-fluid">
                    <div class="row">
                        <div class="col-12">
                            <div class="page-title-box d-sm-flex align-items-center justify-content-between">
                                <h4 class="mb-sm-0 font-size-18">Edit Catatan Intelektual</h4>
                                <div class="page-title-right">
                                    <ol class="breadcrumb m-0">
                                        <li class="breadcrumb-item"><a href="lihat_catatan.php">Lihat Catatan Intelektual</a></li>
                                        <li class="breadcrumb-item active">Edit Catatan Intelektual</li>
                                    </ol>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="card">
                                <div class="card-body">
                                    <form method="POST" action="edit_catatan.php?id=<?php echo $id_calek; ?>" enctype="multipart/form-data">
                                        <div class="form-group">
                                            <label for="judul_calek">Judul Catatan Intelektual</label>
                                            <input type="text" class="form-control" id="judul_calek" name="judul_calek" value="<?php echo htmlspecialchars($judul_calek); ?>" required>
                                        </div>
                                        <div class="form-group">
                                        ...
                                        <div class="form-group">
                                            <label for="thumbnail_calek">Thumbnail</label>
                                            <input type="file" class="form-control" id="userfile" name="userfile[]">
                                            <?php if ($thumbnail_calek) : ?>
                                                <img src="<?php echo $thumbnail_calek; ?>" alt="Thumbnail" class="img-thumbnail mt-2" style="width: 150px;">
                                            <?php endif; ?>
                                        </div>
                                        <button type="submit" class="btn btn-primary">Update</button>
                                        <a href="lihat_catatan.php" class="btn btn-secondary">Kembali</a>
                                    </form>
                                </div>
                            </div>
                        </div>
